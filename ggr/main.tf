...
module "install_ggr" {
  source         = "./modules/install-ggr"
  selenoid_nodes = var.selenoid_nodes
}
...
module "cloud_config" {
  ...
  write_files = concat(
    ...
    module.install_ggr.write_files,
    var.write_files
  )

  runcmd = compact(concat(
    module.install_docker.runcmd,
    module.install_ggr.runcmd,
    var.runcmd
  ))

  final_message = "GGR installation finished"
}
